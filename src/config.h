/* src/config.h.  Generated from config.h.in by configure.  */
/* src/config.h.in.  Generated from configure.ac by autoheader.  */

/* Define to 1 if you have the <inttypes.h> header file. */
#define HAVE_INTTYPES_H 1

/* Define to 1 if you have the `hamlib' library (-lhamlib). */
/* #undef HAVE_LIBHAMLIB */

/* Define to 1 if you have the <memory.h> header file. */
#define HAVE_MEMORY_H 1

/* Define to 1 if you have the <stdint.h> header file. */
#define HAVE_STDINT_H 1

/* Define to 1 if you have the <stdlib.h> header file. */
#define HAVE_STDLIB_H 1

/* Define to 1 if you have the <strings.h> header file. */
#define HAVE_STRINGS_H 1

/* Define to 1 if you have the <string.h> header file. */
#define HAVE_STRING_H 1

/* Define to 1 if you have the <sys/stat.h> header file. */
#define HAVE_SYS_STAT_H 1

/* Define to 1 if you have the <sys/types.h> header file. */
#define HAVE_SYS_TYPES_H 1

/* Define to 1 if you have the <unistd.h> header file. */
#define HAVE_UNISTD_H 1

/* Name of package */
#define PACKAGE "linfax"

/* Define to the address where bug reports for this package should be sent. */
#define PACKAGE_BUGREPORT "vincent.bsod@gmail.com"

/* Define to the full name of this package. */
#define PACKAGE_NAME "linfax"

/* Define to the full name and version of this package. */
#define PACKAGE_STRING "linfax 0.1.2"

/* Define to the one symbol short name of this package. */
#define PACKAGE_TARNAME "linfax"

/* Define to the home page for this package. */
#define PACKAGE_URL ""

/* Define to the version of this package. */
#define PACKAGE_VERSION "0.1.2"

/* Define to 1 if you have the ANSI C header files. */
#define STDC_HEADERS 1

/* Build with support for Advanced Linux Sound System (ALSA). */
#define USE_ALSA 1

/* Build with support for audio files. */
#define USE_FILE 1

/* Build with support for the open sound system (OSS). */
#define USE_OSS 1

/* Build with support for the PTC-II */
/* #undef USE_PTC */

/* Version number of package */
#define VERSION "0.1.2"
